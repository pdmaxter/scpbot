...
// ─────────────────────────────────────────────────────────────────────────────
//  BTC Scalping Strategy  —  EMA(8/21) + RSI(14) + VWAP + ATR(14)
...
class ScalpingStrategy extends EventEmitter {
  constructor (cfg = {}) {
    ...
    this.indicatorSnaps  = [];   // { time, emaFast, emaSlow, vwap, rsi }
  }

  // ── Public ───────────────────────────────────────────────────────────────

  /** Feed one *closed* 5-min candle. Returns current state object. */
  processCandle (candle) {
    ...
    if (dateStr !== this.currentDate) {
      ...
      this.dailyStartCap   = this.capital;
      this.dayTradeStopped = false;
      this.cumVP = 0; this.cumVol = 0;   // VWAP resets daily
    }
    ...
    // ── Incremental ATR (Wilder smoothing) ───────────────────────────────
  ...
  /**
   * Restore indicator state from historical candles without running position
   * management or emitting trade events. Called on server restart so the bot
   * can resume trading immediately after re-connecting to Binance WS.
   *
   * Capital, trades[], and dailyPnl must be pre-populated from MongoDB BEFORE
   * calling this — this method only restores the technical indicator state.
   *
   * @param {Array}  candles          - Closed 5-min candles (200 recommended)
   * @param {String} [todayOverride]  - Force today's date (YYYY-MM-DD)
   */
  restoreFromHistory (candles, todayOverride) {
    if (!candles || candles.length < this.minBars) {
      console.warn(`[Strategy] restoreFromHistory: only ${candles?.length} candles, need ${this.minBars} — will warm up on live data`);
      return false;
    }
    ...
    // ── Recompute VWAP for *today* only (VWAP resets daily) ───────────────
    ...
    // ── Set day context (so day-boundary logic is correct on first live bar)
    this.currentDate = today;

    // ── Volume window for MA ───────────────────────────────────────────────
    this.volWin = candles.slice(-this.volMaPeriod).map(c => c.volume);

    this.warmedUp = true;
    console.log(`[Strategy] Restored from ${candles.length} candles. EMA(8)=${this.emaFast?.toFixed(2)} RSI=${this.rsiVal?.toFixed(1)} ATR=${this.atrVal?.toFixed(2)}`);
    return true;
  }

  /**
   * Set the daily context (start capital + date) — called after restoring
   * trades from DB so today's P&L is computed correctly.
   */
